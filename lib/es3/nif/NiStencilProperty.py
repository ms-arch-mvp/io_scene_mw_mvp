...
class Action(IntEnum):
    ...
    ACTION_DECREMENT = 4
    ACTION_INVERT = 5
    ACTION_INCREMENT_WRAP = 6
    ACTION_DECREMENT_WRAP = 7


def safe_enum(cls, value, default):
    try:
        return cls(value)
    except ValueError:
        print(f"Warning: {value} is not a valid {cls.__name__}, defaulting to {default.name}")
        return default
...
class NiStencilProperty(NiProperty):
    ...
    def load(self, stream):
        ...
        self.stencil_function = safe_enum(TestFunction, stream.read_int(), TestFunction.TEST_NEVER)
        self.stencil_ref = stream.read_uint()
        self.stencil_mask = stream.read_uint()
        self.fail_action = safe_enum(Action, stream.read_int(), Action.ACTION_KEEP)
        self.pass_z_fail_action = safe_enum(Action, stream.read_int(), Action.ACTION_KEEP)
        self.pass_action = safe_enum(Action, stream.read_int(), Action.ACTION_KEEP)
        self.draw_mode = safe_enum(DrawMode, stream.read_int(), DrawMode.DRAW_CCW_OR_BOTH)
